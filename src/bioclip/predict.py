...
class CustomLabelsClassifier(BaseClassifier):
    ...
    @torch.no_grad()
    def predict(self, images: Union[List[str], str, List[PIL.Image.Image]], k: int = None) -> dict[str, float]:
...
class TreeOfLifeClassifier(BaseClassifier):
    ...
    @torch.no_grad()
    def predict(self, images: Union[List[str], str, List[PIL.Image.Image]], rank: Rank, min_prob: float = 1e-9, k: int = 5) -> dict[str, dict[str, float]]:
        if isinstance(images, str):
            images = [images]
        probs = self.create_probabilities_for_images(images, self.txt_features)
        result = []
